...
from diff_gaussian_rasterization_dc import GaussianRasterizationSettings as GaussianRasterizationSettingsDC, GaussianRasterizer as GaussianRasterizerDC
...
def render_with_dc(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    """
    Render the scene. 
    ...
    raster_settings = GaussianRasterizationSettingsDC(
        image_height=int(viewpoint_camera.image_height),
        image_width=int(viewpoint_camera.image_width),
        tanfovx=tanfovx,
        tanfovy=tanfovy,
    ...
    rasterizer = GaussianRasterizerDC(raster_settings=raster_settings)
